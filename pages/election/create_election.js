...
class LoginForm extends Component {
	...
	signin = async event => {
		...
		try {
			...
			if (bool) {
				// --- FIX: You must use the user's email, not 'xyz', to find the contract ---
			...
			this.setState({ errorMess: err.message, loading: false });
		}
	};
		...
		<div className="login-form">
			<style JSX>{`
                .login-form {
                    ...
                    height:100%;
                    position:absolute;
                    background: url('/blockchain.jpg') no-repeat; {/* <-- UPDATED PATH */}
                } 
              `}</style>

			<Grid textAlign="center" style={{ height: '100%' }} verticalAlign="middle">
				<Grid.Column style={{ maxWidth: 380 }}>
					<Form size="large" error={!!this.state.errorMess}> {/* Added error prop */}
						<Segment>
							...
							<Message error header="Oops!" content={this.state.errorMess} />
							<Message icon info>
